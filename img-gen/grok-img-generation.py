...
OUTDIR = Path("images_grok")
CSV_PATH = Path("annotations_grok.csv")
...
def get_xai_api_key():
    load_dotenv()
    api_key = "" # get from https://x.ai
    if not api_key:
        raise RuntimeError("XAI_API_KEY not set. Put it in .env or export it.")
    return api_key
...
def generate_image(api_key: str, prompt: str) -> bytes:
    """
    Generate image using Grok XAI API and return image bytes.
    """
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        try:
            ...
            print(f"[WARN] Grok generation attempt {attempt}/{MAX_ATTEMPTS} failed: {e}. Retrying in {backoff}s...")
            time.sleep(backoff)
    
    raise RuntimeError(f"Grok generation failed after retries: {last_err}")
